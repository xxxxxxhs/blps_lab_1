...
import ru.blps.lab_1.entity.*;
import ru.blps.lab_1.messaging.OrderNotificationMessage;
import ru.blps.lab_1.messaging.StompSenderService;
import ru.blps.lab_1.repository.*;
...
public class OrderService {
    ...
    public OrderService(
        ...
        StompSenderService stompSenderService,
        ...
        this.stompSenderService = stompSenderService;
    ...
    public OrderDto createOrder(CreateOrderRequest request) {
        return inTransaction(() -> {
            ...
            Order order = new Order(
                ...
                restaurant.getAddress(), request.getCity(), request.getDeliveryAddress(),
                request.getPhone(), request.getComment(), OrderStatus.NEW
            );
            request.getItems().forEach(itemData -> {
                double price = itemData.getPrice() == null ? 0.0 : itemData.getPrice();
                order.addItem(new OrderItem(order, itemData.getName(), itemData.getQuantity(), price));
            });
            Order saved = orderRepository.save(order);
            scheduleNotification(
                new OrderNotificationMessage(Recipient.CLIENT, saved.getClientId(), telegramChatId,
                    "Ваш заказ создан!\n" + saved)
            );
            return toDto(saved);
        });
    ...
    public OrderDto acceptOrder(Long orderId) {
        return inTransaction(() -> {
            ...
            if (courier == null) throw new IllegalStateException("Cannot accept order without assigned courier");
            ...
            if (telegramChatId != null && !telegramChatId.isBlank()) {
                ...
                notificationService.send(Recipient.RESTAURANT, saved.getRestaurantId(), telegramChatId,
    ...
    public OrderDto rejectOrder(Long orderId) {
        return inTransaction(() -> {
            ...
            if (rejectedCourier == null) throw new IllegalStateException("Cannot reject order without assigned courier");
            orderCourierDecisionRepository.save(
    ...
    public OrderDto cookOrder(Long orderId) {
        return inTransaction(() -> {
            ...
            Order saved = orderRepository.save(order);
            scheduleNotification(
                new OrderNotificationMessage(Recipient.COURIER, saved.getCourierId(), telegramChatId,
    ...
    public OrderDto cancelOrder(Long orderId) {
        return inTransaction(() -> {
            ...
            if (telegramChatId != null && !telegramChatId.isBlank()) {
                ...
                notificationService.send(Recipient.COURIER, saved.getCourierId(), telegramChatId,
                    "Заказ #" + saved.getId() + " отменён.");
                if (notifyRestaurant) {
                    notificationService.send(Recipient.RESTAURANT, saved.getRestaurantId(), telegramChatId,
                        "Заказ #" + saved.getId() + " отменён.");
    ...
    private void scheduleNotification(OrderNotificationMessage... messages) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                msgs.forEach(stompSenderService::send);
            }
        });
    }
    ...
    private Optional<Courier> selectRandomFreeCourier(Set<Long> excludedCourierIds) {
        List<Courier> freeCouriers = courierRepository.findAll().stream()
            .filter(c -> c.getLogin() != null && !c.getLogin().isBlank())
            .filter(c -> !excludedCourierIds.contains(c.getId()))
            .filter(this::isCourierFree)
            .collect(Collectors.toList());
        return freeCouriers.isEmpty()
            ? Optional.empty()
            : Optional.of(freeCouriers.get(random.nextInt(freeCouriers.size())));
    }

    private boolean isCourierFree(Courier courier) {
        return orderRepository.findTopByCourier_IdOrderByIdDesc(courier.getId())
            .map(o -> FREE_COURIER_LAST_ORDER_STATUSES.contains(o.getStatus()))
            .orElse(true);
    }

    private Set<Long> getRejectedCourierIds(Long orderId) {
        return orderCourierDecisionRepository
            .findByOrder_IdAndDecision(orderId, CourierDecision.REJECTED)
            .stream()
            .map(d -> d.getCourier().getId())
    ...
    private String currentUsername() {
        ...
        if (name == null || name.isBlank()) throw new AccessDeniedException("Not authenticated");
        return name;
    }

    private void assertOrderVisible(Order order) {
        String login = currentUsername();
        if (appUserRepository.findByUsername(login).filter(u -> u.getId().equals(order.getClientId())).isPresent()) return;
        if (courierRepository.findByLogin(login).filter(c -> c.getId().equals(order.getCourierId())).isPresent()) return;
        if (restaurantRepository.findByLogin(login).filter(r -> r.getId().equals(order.getRestaurantId())).isPresent()) return;
    ...
    private void assertClientOwnsOrder(Order order) {
        ...
        if (!client.getId().equals(order.getClientId())) throw new AccessDeniedException("Not your order");
    }
    ...
    private void assertAssignedCourier(Order order) {
        ...
        if (!order.getCourierId().equals(courier.getId())) throw new AccessDeniedException("Not your assigned order");
    }
    ...
    private void assertRestaurantOfOrder(Order order) {
        ...
        if (!restaurant.getId().equals(order.getRestaurantId())) throw new AccessDeniedException("Not your restaurant order");
    }

    private OrderDto toDto(Order order) {
        List<OrderItemDto> itemDtos = order.getItems().stream()
            .map(i -> new OrderItemDto(i.getName(), i.getQuantity()))
            .collect(Collectors.toList());
        return new OrderDto(order.getId(), order.getStatus().name(), order.getRestaurantAddress(),
            order.getCity(), order.getDeliveryAddress(), order.getPhone(), order.getComment(), itemDtos);
    }
}
